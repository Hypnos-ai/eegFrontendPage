...
class TrainingThread(QThread):
    ...
    def __init__(self, dqn_model, combined_file, processed_dir, eeg_manager, 
                 is_sample=False):
        super().__init__()
    ...
    def run(self):
        try:
            ...
            task_names = list(self.eeg_manager.task_names.keys())  
...
class MainWindow(QMainWindow):
    def __init__(self, *args):
        ...
        if not self.action_inputs:
            self.add_new_action()  # Add first action by default
    ...
    def create_control_panel(self, *args):
        ...
        # Initially disable recording buttons until session starts
        ...
        # Add description
        sample_label = QLabel("View pre-recorded sample EEG data:")
        ...
        sample_layout.addWidget(view_sample_btn)
        ...
        load_sample_btn = QPushButton("Load Sample Data as Session")
        load_sample_btn.clicked.connect(self.load_sample_as_session)
        sample_layout.addWidget(load_sample_btn)
        
        # Add sample group to layout before or after other groups
        layout.addWidget(sample_group)
    ...
    def update_plot_channels(self, enabled_channels, *args):
        ...
        try:
            ...
            self.eeg_manager.update_channels(channel_names)
    ...
    def start_session(self, *args):
        ...
        try:
            ...
            self.eeg_manager.task_names = updated_tasks
    ...
    def analyze_session(self, *args):
        ...
        try:
            ...
            try:
                patterns, class_names = self.eeg_manager.analyze_session(
                    self.eeg_manager.current_session_num
                )
    ...
    def toggle_board_connection(self, *args):
        ...
        if not self.eeg_manager.is_streaming:
            port = self.port_select.currentText()
            if self.eeg_manager.initialize_board(port):
            ...
            if self.eeg_manager.stop_stream():
    ...
    def update_session_buttons(self, *args):
        ...
        try:
            ...
            if sessions:
                ...
                self.session_buttons_layout.addWidget(QLabel("Previous Sessions:"))
                for session in sessions:
                    ...
                    # Delete button
                    delete_btn = QPushButton("×")  # Using × symbol
    ...
    def delete_session(self, session_num, *args):
        ...
        try:
            ...
            # Set icon explicitly
            reply_box = QMessageBox()
            reply_box.setIcon(QMessageBox.Question)
            reply_box.setWindowTitle('Delete Session')
            reply_box.setText(f'Are you sure you want to delete Session {session_num}?')
            reply_box.setStandardButtons(QMessageBox.Yes | QMessageBox.No)
            reply_box.setDefaultButton(QMessageBox.No)
            
            # Set stylesheet for the message box
            ...
            if reply_box.exec_() == QMessageBox.Yes:
                self.eeg_manager.delete_session(session_num)
                self.update_session_buttons()
                self.add_status_message(f"Deleted Session {session_num}")
                
                if session_num == self.eeg_manager.current_session_num:
                    self.analyze_btn.setEnabled(False)
                    self.eeg_manager.current_session_num = None
    ...
    def delete_all_sessions(self, *args):
        ...
        try:
            ...
            if reply == QMessageBox.Yes:
                self.eeg_manager.delete_all_sessions()
                self.update_session_buttons()  # Refresh session list
                self.analyze_btn.setEnabled(False)
    ...
    def add_new_action(self, action_key=None, action_name=None, silent=False, *args):
        """Add a new action input row with UUID"""
        ...
        name_input.textChanged.connect(lambda: self.update_eeg_manager_tasks())
        ...
        if not silent:
            self.add_status_message(f"Added {action_name}")
            self.update_eeg_manager_tasks()
    ...
    def update_eeg_manager_tasks(self, *args):
    ...
    def view_sample_data(self, index, *args):
        ...
        try:
            ...
            plot_window = QWidget()
            plot_window.setWindowTitle(f"Sample Data Visualization - {sample_files[0].name}")
    ...
    def train_dqn_model(self, *args):
        ...
        try:
            ...
            if self.eeg_manager.current_session_num == 0:  # Sample data
                combined_file = os.path.join(self.eeg_manager.sample_dir, 'formatted', 'S03.fif')
            else:
                # Regular session - combine files first
                self.training_progress.append("Combining FIF files...")
                try:
                    combined_file = self.eeg_manager.combine_fif_files()
            ...
            self.training_thread = TrainingThread(
                dqn_model, 
                combined_file, 
                self.eeg_manager.processed_dir,
                is_sample=(self.eeg_manager.current_session_num == 0),
                eeg_manager=self.eeg_manager
            )
    ...
    def apply_filters(self, data, *args):
        ...
        try:
            ...
            board_id = self.eeg_manager.board.get_board_id()
    ...
    def load_sample_as_session(self, *args):
        ...
        try:
            ...
            sample_file = os.path.join(self.eeg_manager.sample_dir, 'formatted', 'S03.fif')
            ...
            self.eeg_manager.current_session_num = 0  # Use 0 to indicate sample data
            self.analyze_btn.setEnabled(True)
    ...
    def add_channel(self, name, layout, *args):
        """Add a new channel with UUID"""
        try:
            # Validate channel name
            valid_channels = {'Fp1', 'Fp2', 'F7', 'F3', 'Fz', 'F4', 'F8', 'T3', 'C3', 'Cz', 'C4', 'T4', 'T5', 'P3', 'Pz', 'P4', 'T6', 'O1', 'O2', 'FCz', 'CPz', 'C2'}  # Add more as needed
            if name not in valid_channels:
                self.add_status_message(f"Error: '{name}' is not a valid EEG channel in the 10-20 system")
                return

            # Create channel row with better spacing
            channel_row = QWidget()
            ...
            # Create name input with better styling
            ...
            """)
            # Connect textChanged signal to update plots immediately
            name_input.textChanged.connect(lambda: self.update_plot_channels(self.channel_configs))
            row_layout.addWidget(name_input, stretch=2)
            
            # Create enable/disable checkbox with better styling
            enable_box = QCheckBox("Enable")
            enable_box.setChecked(True)
            enable_box.setStyleSheet("""
                QCheckBox {
                    color: white;
                    padding: 5px;
                }
            """)
            # Connect stateChanged signal to update plots immediately
            enable_box.stateChanged.connect(lambda: self.update_plot_channels(self.channel_configs))
            ...
            self.add_status_message(f"Error adding channel: {str(e)}")
